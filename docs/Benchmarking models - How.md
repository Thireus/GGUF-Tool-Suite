...
Let's get an idea about how long benchmarking will take... This command returns how many benchmarking rounds (tensors to benchmark + 1) the `benchmark_each_tensor.sh` script will perform:
...
* **Incompatible `llama-perplexity` flags**: `PPL_COMMAND_TEMPLATE` must not contain `-mqkv`/`--merge-qkv`, `-muge`/`--merge-up-gate-experts`, `-rtr`/`--run-time-repack` or `-khad`/`--k-cache-hadamard` when `--hotswap` is used - these transform or merge weights at load time in ways the hot-swap machinery cannot follow, so the script refuses to start and explains why. `--no-mmap` is strongly recommended (with mmap enabled, same-quant swaps cannot be refreshed in place).
* **Automatic full-reload fallback**: some tensors are fused, merged or derived at load time and cannot be hot-swapped in place (llama-perplexity refuses them loudly, or reports that derived state could not be refreshed). When a round cannot be completed correctly via hot-swap, the script automatically retries it once with a **full model reload** (fresh `llama-perplexity` process): the swapped shards are then part of the initial load, so every load-time derivation re-runs with the new data and the result is correct by construction - at the cost of one model load for that round. The script also *learns* which tensors are unswappable and sends those rounds straight to a full reload, without wasting a computation on a doomed hot-swap attempt first. As a consequence, `.txt.failed` quarantine files only remain when **both** the hot-swap attempt and the full-reload retry failed.
...
* Some models ship tensors that `llama-perplexity` ignores at load time (logged as `model has unused tensor ... -- ignoring`, e.g. NextN/MTP layers such as GLM's last block). Swapping them has no effect on PPL/KLD, so benchmarking them only pollutes the calibration data with baseline-identical values. The script detects them automatically in hotswap mode: their rounds are skipped without wasting a computation and a `bench_..._result.*.txt.unused` note is written instead of a result file. Lock these tensors in `USER_REGEX` (append `=locked`) to silence the warnings for good.
...
From there, you will need to assess how far in this list you can go while still being able to load models quantized to these types. For example, if your `BASELINE_QTYPE` is `q5_K` then you probably won't be able to load and benchmark `q5_1` and beyond since their bpw is higher than your baseline quantization type (which is already the max your hardware supports). If you decide to change your `BASELINE_QTYPE` for this benchmark, remember you will also need to edit both the `USER_REGEX` found under the "# 3. USER_REGEX" section to match the baseline quantization type for each tensor and the `BASELINE_QTYPE` found under the "# 8. Baseline QTYPE for baseline PPL+KLD computation" section of the `benchmark_each_tensor.sh` script - therefore, I strongly recommend to reconfigure the benchmark script by following the [Configure the benchmark script](https://github.com/Thireus/GGUF-Tool-Suite/blob/main/docs/Benchmarking%20models%20-%20How.md#configure-the-benchmark-script) section of this guide again using the new `BASELINE_QTYPE` env var.
