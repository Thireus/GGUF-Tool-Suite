...
#** DeepSeek-TNG-R1T2-Chimera-THIREUS-ANY-SPECIAL-SMOLER.sh   **#
#** used for iq1_s* qtypes only. Adjust $1 in $custom!        **#
#**                                                           **#
#** ********************************************************* **#
#** --------------- Updated: Nov-06-2025 -------------------- **#
#** ********************************************************* **#
...
# GPU Only - not divisible by 256 so only supports qN_0 - forcing it to be iq1_s so that llama-quantize doesn't stop
blk\.([0-9]|[1-5][0-9]|60)\.attn_k_b\.weight=iq1_s
...
mkdir DeepSeek-TNG-R1T2-Chimera-THIREUS-${1^^}-SPECIAL_SPLIT/ && llama-quantize --keep-split \
    ...
    ${1^^} \
    $(nproc) && chmod 444 DeepSeek-TNG-R1T2-Chimera-THIREUS-${1^^}-SPECIAL_SPLIT/*.gguf || echo "ERROR: Something went wrong, please check the directory doesn't already exist and that you have sufficient available disk space!"
