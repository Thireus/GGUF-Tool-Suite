...
#** DeepSeek-V3-0324-THIREUS-ANY-SPECIAL-SMOL.sh used for     **#
#** 1-bit qtypes only except iq1_s*. Adjust $1 in $custom!    **#
#**                                                           **#
#** ********************************************************* **#
#** --------------- Updated: Nov-06-2025 -------------------- **#
#** ********************************************************* **#
...
# echo "$(for f in `ls DeepSeek-V3-0324-DQ4_K_R4-*.gguf`; do gguf_info.py "$f"; done)" | grep 'dtype=' | awk -F $'\t' '{print $1 "=" $3}' | sed 's/=dtype=/=/g' | sed 's/\./\\./g'
custom="
...
## Special attention kernels — single-quant only (llama-quantize takes care of it) - forcing it to be iq1_m so that llama-quantize doesn't stop
...
# DeepSeek-V3-0324-THIREUS-TEMPLATE.gguf is too big and not worth using it because Q8_0 quanitsation is fast!
mkdir DeepSeek-V3-0324-THIREUS-${1^^}-SPECIAL_SPLIT/ && llama-quantize --keep-split \
    --custom-q "$custom" \
    --imatrix imatrix_ubergarm.dat \
    DeepSeek-V3-0324-THIREUS-BF16-SPECIAL_SPLIT/DeepSeek-V3-0324-THIREUS-BF16-SPECIAL_TENSOR-00001-of-01148.gguf \
    DeepSeek-V3-0324-THIREUS-${1^^}-SPECIAL_SPLIT/DeepSeek-V3-0324-THIREUS-${1^^}-SPECIAL_TENSOR.gguf \
    ${1^^} \
    $(nproc) && chmod 444 DeepSeek-V3-0324-THIREUS-${1^^}-SPECIAL_SPLIT/*.gguf || echo "ERROR: Something went wrong, please check the directory doesn't already exist and that you have sufficient available disk space!"
