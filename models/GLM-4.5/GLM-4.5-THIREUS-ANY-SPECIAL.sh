...
#** GLM-4.5-THIREUS-ANY-SPECIAL.sh used for ppl benchmarking  **#
...
mkdir GLM-4.5-THIREUS-${1^^}-SPECIAL_SPLIT/ && llama-quantize --keep-split \
    ...
    ${1^^} \
    $(nproc) && chmod 444 GLM-4.5-THIREUS-${1^^}-SPECIAL_SPLIT/*.gguf || echo "ERROR: Something went wrong, please check the directory doesn't already exist and that you have sufficient available disk space!"
