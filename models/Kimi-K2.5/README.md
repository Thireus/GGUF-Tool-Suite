...
This repository provides **GGUF-quantized tensors** for the Kimi-K2.5 model (official repo: https://huggingface.co/moonshotai/Kimi-K2.5). These GGUF shards are designed to be used with **Thireus’ GGUF Tool Suite** (https://github.com/Thireus/GGUF-Tool-Suite), a collection of tools that automatically finds the perplexity-optimal mix of quantizations for any given a model size target. With this GGUF Tool Suite, you can produce your own Dynamic 3.0 Quants recipes and achieve optimum accuracy & SOTA quantization performance. Give it a try here: https://gguf.thireus.com/quant_assign.html  
...
cp -f models/Kimi-K2.5/download.conf . # Use the download.conf of the chosen model
mkdir -p kitchen && cd kitchen
# Obtain a recipe example for the chosen model from ../recipe_examples/
../quant_downloader.sh ../recipe_examples/ik_harmonized_recipes/Kimi-K2.5.ROOT-1.9968bpw-3.8128ppl.238GB-GGUF_10GB-GPU_228GB-CPU.90e3c2f_4766d51.recipe
...
~/ik_llama.cpp/build/bin/llama-cli \
  -m Kimi-K2.5-THIREUS-BF16-SPECIAL_TENSOR-00001-of-01148.gguf \
...
Usage example:
```
./llama-quantize --keep-split --imatrix imatrix_ubergarm.dat --individual-tensors 2,3,1094 Kimi-K2.5-THIREUS-BF16-SPECIAL_TENSOR-00001-of-01097.gguf my_new_shards.gguf iq3_s 12
```
