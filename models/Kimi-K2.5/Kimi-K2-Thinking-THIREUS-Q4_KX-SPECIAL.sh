...
#** Kimi-K2.5-THIREUS-Q4_KX-SPECIAL.sh used with patch        **#
#** Q4_K, cf https://github.com/ggml-org/llama.cpp/pull/17064 **#
#**                                                           **#
#** ********************************************************* **#
#** --------------- Updated: Mar-12-2026 -------------------- **#
#** ********************************************************* **#
...
^blk\.([1-9]|[1-5][0-9]|60)\.ffn_down_exps\.weight$=q4_K

# ffn_up_exps — up-projection (per-expert) — qbits: 16
^blk\.([1-9]|[1-5][0-9]|60)\.ffn_up_exps\.weight$=q4_K

# ffn_gate_exps — gating network (per-expert) — qbits: 16
^blk\.([1-9]|[1-5][0-9]|60)\.ffn_gate_exps\.weight$=q4_K
"
...
mkdir Kimi-K2.5-THIREUS-${1^^}-SPECIAL_SPLIT/ && llama-quantize --keep-split \
    --custom-q "$custom" \
    Kimi-K2.5-THIREUS-BF16-SPECIAL_SPLIT/Kimi-K2.5-THIREUS-BF16-SPECIAL_TENSOR-00001-of-01097.gguf \
    Kimi-K2.5-THIREUS-${1^^}-SPECIAL_SPLIT/Kimi-K2.5-THIREUS-${1^^}-SPECIAL_TENSOR.gguf \
    ${1^^} \
    $(nproc) && chmod 444 Kimi-K2.5-THIREUS-${1^^}-SPECIAL_SPLIT/*.gguf || echo "ERROR: Something went wrong, please check the directory doesn't already exist and that you have sufficient available disk space!"

# TODO - MANUALLY - Merge the exps ggufs into a classic q4_K (and q4_k_r4) produced with Kimi-K2.5-THIREUS-UPPER-SPECIAL.sh and main branch
# 1. Rename the repo Kimi-K2.5-THIREUS-Q4_KX-SPECIAL_SPLIT
...
# cat Kimi-K2.5-THIREUS-BF16-SPECIAL_SPLIT/tensors.map | grep '_exps\.' | cut -d: -f1 | sed 's/\-BF16-/-Q4_K-/'
# 3. Move files to Kimi-K2.5-THIREUS-Q4_K-SPECIAL_SPLIT (classic q4_0 produced with Kimi-K2.5-THIREUS-UPPER-SPECIAL.sh):
# for f in $(cat Kimi-K2.5-THIREUS-BF16-SPECIAL_SPLIT/tensors.map | grep '_exps\.' | cut -d: -f1 | sed 's/\-BF16-/-Q4_K-/'); do mv -f Kimi-K2.5-THIREUS-Q4_KX-SPECIAL_SPLIT/$f Kimi-K2.5-THIREUS-Q4_K-SPECIAL_SPLIT/; done;
# 4. You now can safely get rid of Kimi-K2.5-THIREUS-Q4_KX-SPECIAL_SPLIT