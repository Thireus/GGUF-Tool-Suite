...
#** DeepSeek-V3.1-Terminus-THIREUS-ANY-SPECIAL.sh used for    **#
...
# echo "$(for f in `ls DeepSeek-V3.1-Terminus-DQ4_K_R4-*.gguf`; do gguf_info.py "$f"; done)" | grep 'dtype=' | awk -F $'\t' '{print $1 "=" $3}' | sed 's/=dtype=/=/g' | sed 's/\./\\./g'
custom="
...
# DeepSeek-V3.1-Terminus-THIREUS-TEMPLATE.gguf is too big and not worth using it because Q8_0 quanitsation is fast!
mkdir DeepSeek-V3.1-Terminus-THIREUS-${1^^}-SPECIAL_SPLIT/ && llama-quantize --keep-split \
    --custom-q "$custom" \
    --imatrix imatrix_ubergarm.dat \
    DeepSeek-V3.1-Terminus-THIREUS-BF16-SPECIAL_SPLIT/DeepSeek-V3.1-Terminus-THIREUS-BF16-SPECIAL_TENSOR-00001-of-01087.gguf \
    DeepSeek-V3.1-Terminus-THIREUS-${1^^}-SPECIAL_SPLIT/DeepSeek-V3.1-Terminus-THIREUS-${1^^}-SPECIAL_TENSOR.gguf \
    ${1^^} \
    $(nproc) && chmod 444 DeepSeek-V3.1-Terminus-THIREUS-${1^^}-SPECIAL_SPLIT/*.gguf || echo "ERROR: Something went wrong, please check the directory doesn't already exist and that you have sufficient available disk space!"
