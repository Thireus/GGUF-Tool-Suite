...
#** Qwen3.5-2B-THIREUS-ANY-SPECIAL-SMOL.sh used for iq1_s*.   **#
...
# Qwen3.5-2B-THIREUS-TEMPLATE.gguf is too big and not worth using it because Q8_0 quanitsation is fast!
mkdir Qwen3.5-2B-THIREUS-${1^^}-SPECIAL_SPLIT/ && llama-quantize --keep-split \
    --custom-q "$custom" \
    --imatrix imatrix_ubergarm.dat \
    --ignore-imatrix-rules \
    Qwen3.5-2B-THIREUS-BF16-SPECIAL_SPLIT/Qwen3.5-2B-THIREUS-BF16-SPECIAL_TENSOR-00001-of-00321.gguf \
    Qwen3.5-2B-THIREUS-${1^^}-SPECIAL_SPLIT/Qwen3.5-2B-THIREUS-${1^^}-SPECIAL_TENSOR.gguf \
    ${1^^} \
    $(nproc) && chmod 444 Qwen3.5-2B-THIREUS-${1^^}-SPECIAL_SPLIT/*.gguf || echo "ERROR: Something went wrong, please check the directory doesn't already exist and that you have sufficient available disk space!"
