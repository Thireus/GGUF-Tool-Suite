...
#** Qwen3-4B-Thinking-2507-THIREUS-ANY-SPECIAL_SMOL.sh used   **#
#** for iq2_x* and iq2_s* qyptes only. Adjust $1 in $custom!  **#
#**                                                           **#
#** ********************************************************* **#
#** --------------- Updated: Jan-22-2026 -------------------- **#
#** ********************************************************* **#
...
blk\.([0-9]|[1-2][0-9]|3[0-5])\.attn_v\.weight=$1
blk\.([0-9]|[1-2][0-9]|3[0-5])\.attn_output\.weight=$1
blk\.([0-9]|[1-2][0-9]|3[0-5])\.attn_k_norm\.weight=f32
blk\.([0-9]|[1-2][0-9]|3[0-5])\.attn_q\.weight=$1
blk\.([0-9]|[1-2][0-9]|3[0-5])\.attn_norm\.weight=f32
blk\.([0-9]|[1-2][0-9]|3[0-5])\.attn_k\.weight=$1
blk\.([0-9]|[1-2][0-9]|3[0-5])\.attn_q_norm\.weight=f32

## Core FFN weights
blk\.([0-9]|[1-2][0-9]|3[0-5])\.ffn_gate\.weight=$1
blk\.([0-9]|[1-2][0-9]|3[0-5])\.ffn_norm\.weight=f32
blk\.([0-9]|[1-2][0-9]|3[0-5])\.ffn_down\.weight=$1
blk\.([0-9]|[1-2][0-9]|3[0-5])\.ffn_up\.weight=$1
"
...
# Qwen3-4B-Thinking-2507-THIREUS-TEMPLATE.gguf is too big and not worth using it because Q8_0 quanitsation is fast!
mkdir Qwen3-4B-Thinking-2507-THIREUS-${1^^}-SPECIAL_SPLIT/ && llama-quantize --keep-split \
    --custom-q "$custom" \
    --imatrix imatrix_ubergarm.dat \
    Qwen3-4B-Thinking-2507-THIREUS-BF16-SPECIAL_SPLIT/Qwen3-4B-Thinking-2507-THIREUS-BF16-SPECIAL_TENSOR-00001-of-00399.gguf \
    Qwen3-4B-Thinking-2507-THIREUS-${1^^}-SPECIAL_SPLIT/Qwen3-4B-Thinking-2507-THIREUS-${1^^}-SPECIAL_TENSOR.gguf \
    ${1^^} \
    $(nproc) && chmod 444 Qwen3-4B-Thinking-2507-THIREUS-${1^^}-SPECIAL_SPLIT/*.gguf || echo "ERROR: Something went wrong, please check the directory doesn't already exist and that you have sufficient available disk space!"
