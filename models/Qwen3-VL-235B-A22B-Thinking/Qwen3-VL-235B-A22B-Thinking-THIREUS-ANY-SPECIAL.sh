...
# Qwen3-VL-235B-A22B-Thinking-THIREUS-TEMPLATE.gguf is too big and not worth using it because Q8_0 quanitsation is fast!
mkdir Qwen3-VL-235B-A22B-Thinking-THIREUS-${1^^}-SPECIAL_SPLIT/ && llama-quantize --keep-split \
    ...
    ${1^^} \
    $(nproc) && chmod 444 Qwen3-VL-235B-A22B-Thinking-THIREUS-${1^^}-SPECIAL_SPLIT/*.gguf || echo "ERROR: Something went wrong, please check the directory doesn't already exist and that you have sufficient available disk space!"
