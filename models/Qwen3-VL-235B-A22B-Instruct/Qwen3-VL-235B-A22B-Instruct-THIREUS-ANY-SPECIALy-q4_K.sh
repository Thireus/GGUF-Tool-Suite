...
#** Qwen3-VL-235B-A22B-Instruct-THIREUS-ANY-SPECIAL.sh used   **#
#** for q4_K qypte only. Adjust $1 in $custom!                **#
#**                                                           **#
#** ********************************************************* **#
#** --------------- Updated: Nov-21-2025 -------------------- **#
#** ********************************************************* **#
...
# ffn_down_exps (down-extraction) — qbits: 16 - q4_k_r4 used to fix "found nan value at block 30" issue
^blk\.([0-9]|[1-4][0-9]|5[0-8]|[6-8][0-9]|9[0-3])\.ffn_down_exps\.weight$=$1
^blk\.(59)\.ffn_down_exps\.weight$=q4_k_r4
...
# Qwen3-VL-235B-A22B-Instruct-THIREUS-TEMPLATE.gguf is too big and not worth using it because Q8_0 quanitsation is fast!
mkdir Qwen3-VL-235B-A22B-Instruct-THIREUS-${1^^}-SPECIAL_SPLIT/ && llama-quantize --keep-split \
    --custom-q "$custom" \
    --imatrix imatrix_ubergarm.dat \
    Qwen3-VL-235B-A22B-Instruct-THIREUS-BF16-SPECIAL_SPLIT/Qwen3-VL-235B-A22B-Instruct-THIREUS-BF16-SPECIAL_TENSOR-00001-of-01132.gguf \
    Qwen3-VL-235B-A22B-Instruct-THIREUS-${1^^}-SPECIAL_SPLIT/Qwen3-VL-235B-A22B-Instruct-THIREUS-${1^^}-SPECIAL_TENSOR.gguf \
    ${1^^} \
    $(nproc) && chmod 444 Qwen3-VL-235B-A22B-Instruct-THIREUS-${1^^}-SPECIAL_SPLIT/*.gguf || echo "ERROR: Something went wrong, please check the directory doesn't already exist and that you have sufficient available disk space!"
