...
#** DeepSeek-V2-Lite-THRIEUS-bench.sh is a helper script that **#
#** creates DeepSeek-V2-Lite quants used for benchmarking.    **#
#**                                                           **#
#** ********************************************************* **#
#** --------------- Updated: Jul-10-2025 -------------------- **#
#** ********************************************************* **#
...
#**    し―-J                                                   **#
#**                                                           **#
#** Copyright © 2025 - Thireus.               ₜᵧₚᵢₙ𝓰 > ₜₕᵢₙₖᵢₙ𝓰 **#
#***************************************************************#
...
# ~/gguf-tools/gguf-tools  show DeepSeek-V2-Lite-BF16-THIREUS.gguf  | grep ' tensor ' | cut -d' ' -f 1-3 | awk '{print $3 "=" $1}' | sed 's/\./\\./g'
custom="
# Use q8_0 - attn_kv_b was missing from unsloth!
blk\.([0-9]|1[0-9]|2[0-6])\.attn_kv_b\.weight=q8_0
blk\.([0-9]|1[0-9]|2[0-6])\.attn_kv_a_norm\.weight=q8_0
blk\.([0-9]|1[0-9]|2[0-6])\.ffn_norm\.weight=q8_0
blk\.([0-9]|1[0-9]|2[0-6])\.attn_norm\.weight=q8_0
blk\.([1-9]|1[0-9]|2[0-6])\.ffn_gate_inp\.weight=q8_0
output\.weight=q8_0
...
# GPU Only - All other attention q8_0
blk\.([0-9]|1[0-9]|2[0-6])\.attn_k_b\.weight=q8_0
blk\.([0-9]|1[0-9]|2[0-6])\.attn_v_b\.weight=q8_0
blk\.([0-9]|1[0-9]|2[0-6])\.attn_kv_a_mqa\.weight=q8_0
blk\.([0-9]|1[0-9]|2[0-6])\.attn_output\.weight=q8_0
blk\.([0-9]|1[0-9]|2[0-6])\.attn_q\.weight=q8_0
...
blk\.([1-9]|1[0-9]|2[0-6])\.ffn_down_shexp\.weight=q8_0
blk\.([1-9]|1[0-9]|2[0-6])\.ffn_gate_shexp\.weight=q8_0
blk\.([1-9]|1[0-9]|2[0-6])\.ffn_up_shexp\.weight=q8_0

# MoE Experts (3-60) (CPU) - ffn_down_exps cannot be divided by 256
blk\.([1-9]|1[0-9]|2[0-6])\.ffn_down_exps\.weight=q8_0

blk\.([1-9]|1[0-9]|2[0-6])\.ffn_gate_exps\.weight=$1
blk\.([1-9]|1[0-9]|2[0-6])\.ffn_up_exps\.weight=$1
"
...
#echo "$custom" | grep -v '^#' | grep -v '^$' | shorten_regex_list | optimise_regex_list | sort -t '.' -k3,3
...
~/ik_llama-main-b3856-d185f1f-bin-win-cuda-12.8-x64/llama-quantize \
    --custom-q "$custom" \
    --imatrix imatrix-dummy.dat \
    DeepSeek-V2-Lite-BF16-THIREUS.gguf \
    DeepSeek-V2-Lite-THIREUS-$1.gguf \
    $1 \
    36
