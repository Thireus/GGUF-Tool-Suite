...
DEFAULTS: Dict[str, Any] = {
    ...
    "synergistic_tensors": [],
    "per_tensor_degradation_scaling": None,
...
def emit_parameters(params: Dict[str, Any]) -> str:
    ...
    lines.append("")
    lines.append("# tensors map file computation, imatrix controls and fallback qtype filters")
    ...
    lines.append('fallback_quants = [{}]   #@param {{type:"raw"}}'.format(", ".join('"{}"'.format(q) for q in params["fallback_quants"])))
...
def parse_recipe_to_params(recipe_text: str) -> Dict[str, Any]:
    ...
    if 'harmonization_technique' in params:
        try:
            ...
            pass
    if 'per_tensor_degradation_scaling' in params:
        try:
